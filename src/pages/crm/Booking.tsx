...
import { appointments, type Appointment, patients } from "./mockData";
...
export default function Booking() {
  ...
  const storeOptions = useMemo(() => {
    const values = Array.from(new Set(appointments.map((a) => a.store).filter(Boolean))).sort();
    return ["全部门店", ...values];
  }, []);

  const statusOptions = useMemo(() => {
    const values = Array.from(new Set(appointments.map((a) => a.status).filter(Boolean))).sort();
    return ["全部状态", ...values];
  }, []);

  const sourceOptions = useMemo(() => {
    const values = Array.from(new Set(appointments.map((a) => a.source).filter(Boolean))).sort();
    return ["全部来源", ...values];
  }, []);
        ...
        <div className="border-b border-gray-100 p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 active:bg-gray-100">
                日历视图
              </button>
              <button className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 active:bg-gray-100">
              ...
              <button className="rounded-xl bg-primary-500 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-600 active:bg-primary-700">
                <span className="inline-flex items-center gap-2">
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button className="rounded-xl bg-primary-50 px-3 py-2 text-xs font-semibold text-primary-600 hover:bg-primary-100 active:bg-primary-200">
                        到诊录入
                      </button>
                    </div>
                  </td>
