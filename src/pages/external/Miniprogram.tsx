...
type PageId = "login" | "home" | "notifications" | "store-select" | "appointment" | "profile" | "family-group" | "family-group-detail" | "patient-list" | "my-appointments" | "appointment-detail" | "archive-new";
...
export default function Miniprogram() {
  ...
  const showTabBar = currentPage !== "login" && currentPage !== "notifications" && currentPage !== "archive-new";
  ...
  const [archiveForm, setArchiveForm] = useState(createEmptyArchiveForm);
  ...
  const goBack = () => {
    ...
    else if (currentPage === "archive-new") setCurrentPage("home");
  ...
  const openArchiveNew = () => {
    const storeId = getQueryParam("storeId") || getQueryParam("store") || "惟爱·上海海华医院";
    setArchiveStoreId(storeId);
    setArchiveForm(createEmptyArchiveForm());
    navigateTo("archive-new");
  };

  const submitArchive = () => {
    const name = archiveForm.name.trim();
    const ageValue = archiveForm.age.trim();
    const age = Number.parseInt(ageValue, 10);
    const gender = archiveForm.gender;
    const mobile = archiveForm.mobile.replaceAll(/\s+/g, "");
    const idCard = archiveForm.idCard.trim();

    if (!name) return showToast("请填写姓名");
    if (!ageValue || Number.isNaN(age) || age <= 0) return showToast("请填写正确年龄");
    if (!gender) return showToast("请选择性别");
    if (!mobile) return showToast("请填写手机号");
    if (!/^1\d{10}$/.test(mobile)) return showToast("手机号格式不正确");
    if (archiveForm.eyeSurgerySide !== "无" && !archiveForm.eyeSurgeryDesc.trim()) return showToast("请填写手术史");
    ...
    <div className="min-h-full bg-gray-50 pb-24">
      ...
      <div className="p-2 mt-4 space-y-3">
        <div className="bg-white rounded-xl shadow-sm overflow-hidden mx-2">
          ...
          <div className="border-b border-gray-50 p-4 flex justify-between items-center active:bg-gray-50" onClick={() => navigateTo("patient-list")}>
            <div className="flex items-center gap-3">
    ...
    <div className="bg-gray-200 h-screen w-screen flex justify-center items-center font-sans">
      <div className="w-full h-full sm:w-[375px] sm:h-[812px] bg-gray-50 flex flex-col relative sm:rounded-[40px] sm:border-[14px] border-gray-900 sm:shadow-2xl overflow-hidden">
        ...
        >
          ...
          {currentPage === "archive-new" && (
            <div className="min-h-full flex flex-col bg-white">
              <MiniTopBar title="新增档案" onBack={goBack} />
              <div className="flex-1 bg-white">
                ...
                <div className="px-4 py-2 pt-0">
                  ...
                  <div className="flex gap-6 items-center py-2">
                    <button
                      type="button"
                      onClick={() => setArchiveForm((p) => ({ ...p, gender: "男" }))}
                    ...
                    <button
                      type="button"
                      onClick={() => setArchiveForm((p) => ({ ...p, gender: "女" }))}
                      className="flex items-center gap-2"
                    >
                      <div
                        className={clsx(
                ...
                <div className="px-4 py-4 pt-0">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setArchiveForm((p) => ({
                        ...p,
                        eyeSurgerySide: e.target.value as "无" | "左眼" | "右眼" | "双眼",
          ...
          <div className="absolute inset-0 z-[60]">
            ...
            <div className="absolute bottom-0 w-full bg-white rounded-t-2xl p-6 transform transition-transform duration-300">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setIsAddPatientOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-400">
                  <X weight="bold" />
                </button>
              </div>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input type="text" placeholder="请输入真实姓名" className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all placeholder:text-gray-300" />
                </div>

                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">性别 <span className="text-red-500">*</span></label>
                  <div className="flex gap-6 items-center py-2">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type="text" placeholder="年 / 月 / 日" className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all placeholder:text-gray-300" />
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">身份证号</label>
                  <input type="text" placeholder="选填，用于医保挂号" className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all placeholder:text-gray-300" />
                </div>
              </div>

              <div className="flex gap-3 mt-8 pb-4">
                <button onClick={() => setIsAddPatientOpen(false)} className="flex-[0.4] bg-gray-50 text-gray-600 py-3.5 rounded-xl font-bold active:bg-gray-100 transition-colors">
                  取消
                </button>
                <button onClick={() => {
                  showToast("添加成功");
